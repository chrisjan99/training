...
# to train
model = tflearn.DNN(network, checkpoint_path='../models/model_googlenet', max_checkpoints=1, tensorboard_verbose=2)
#model.fit(X, Y, n_epoch=5, validation_set=0.1, shuffle=True, show_metric=True, batch_size=32, snapshot_step=200, snapshot_epoch=False, run_id='googlenet_koi')
model.fit(X_train, X_label, n_epoch=500, validation_set=(Y_test, Y_label), shuffle=True, show_metric=True, batch_size=54, snapshot_step=200, snapshot_epoch=False, run_id='googlenet_koi')
model.save("../models/koi-googlenet-classifier.tfl")
